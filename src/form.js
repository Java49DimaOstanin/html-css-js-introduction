...
const showListElement = document.querySelector(".show-emploees");
...
function show(index) {
    ...
    if(index== 1){
        const employeesAr = company.getAllemployees();
        showListElement.innerHTML = getEmploeeys(employeesAr)
    }
}
function Company(){
    this.employees = [];
...
Company.prototype.hireEmployees = function(employee) {
...
function getEmploeeys(emploees) {
    const arEmploee = emploees.map(employee => 
         `<div class="full-data">
          <li> Name : ${employee.employee_name} </li>
          <li> Birth date : ${employee.birtDate} </li>
          <li> Email  :  ${employee.email} </li>
          <li> Departament : ${employee.departament} </li>
          <li> Salary : ${employee.salary} </li>
         </div>` );
     return arEmploee.join('');
 }
 console.log(company);